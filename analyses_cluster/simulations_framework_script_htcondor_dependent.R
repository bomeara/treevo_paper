...
analysisNames_Results <- sapply(filesResults,function(x){
...
# for now, just crash if the analyses names don't perfectly lineup with 
	# the full list of independent analyses
matchedAnalysisNames <- match(analysesNames[whichIndependentPrevRun], 
	analysisNames_Results)
if(any(is.na(matchedAnalysisNames))){
	stop("Result files not found for all indep analyses")
	}
# need to load results sequentially
	# place as a sub-object in the pre-existing list: analysisOutput
for(indepName in analysesNames[whichIndependentPrevRun]){
	load(filesResults[analysisNames_Results == indepName])
	analysisOutput[[indepName]] <- results
	}
#############################
# dependent analyses
########################################
